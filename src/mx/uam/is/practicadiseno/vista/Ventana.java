...
public class Ventana extends JFrame implements Observador {
	...
	public Ventana(Manejador manejador) {
		...
		initialize();
		actualiza();
		manejador.agregaObservador(observador);
	}

	@Override
	public void actualiza() {getJList().setListData(manejador.dameDatos());}
	/**
	...
	private void initialize() {
		...
		this.addWindowListener(new java.awt.event.WindowAdapter() {
			public void windowClosing(java.awt.event.WindowEvent e) {
				...
				manejador.quitaObservador(observador);
				manejador.finaliza();
			}
		});
	}
		...
		if (jButtonBorrar == null) {
			...
			jButtonBorrar.addActionListener(e -> {
				...
				if(getJList().getSelectedValue()!= null) {
					String seleccionado = getJList().getSelectedValue();
